...
const userRoles = useSelector(state => state.users.roles)
    const hasAccessToLocalise = userRoles?.indexOf('USERS_LIST') > -1;
  return (
    <div>
      <IconButton
        aria-label="more"
      ...
      >
           ...
           { hasAccessToLocalise && rowData.articleType === 'Global' && !rowData.marketValueFromDB
           &&
            <MenuItem value={'Localise'}  onClick={e => handleMenuItemClick('Localise', rowData)}>Localise</MenuItem>
            } 
      </Menu>
    </div>
  );
